from HPNav_agent import HPNavAgent, name2id
...
parser.add_argument("--title", type=str, required=False, default="/home/ros/kjx/HSPNav/result")
...
parser.add_argument("--max_transition", type=int, default=3000)
parser.add_argument("--start_replay", type=int, default=50)
...
parser.add_argument("--loss_thin", type=int, default=10)
...
def main():
    ...
    if args.checkpoint != "":
        ...
        save_dir = '/home/ros/kjx/HSPNav/result/finetune_agent'
        if os.path.exists(save_dir):
            assert False, "Dir exists!"
        os.makedirs(save_dir)
        # log_dir = os.path.join(args.log_dir, args.title)
        log_dir = '/home/ros/kjx/HSPNav/run/finetune_agent'
    ...
    if args.checkpoint == '':
        global_step = 0
        train_scalars = {'all': ScalarMeanTracker()} #数据检测
    ...
    while global_step < args.max_transition:
        ...
        while not agent.done:
            ...
            print("avg_q:",float(torch.max(agent.q_map)))
