#!/home/cbl/anaconda3/envs/ros_env/bin/python
#!coding:utf-8
...
def color_map(N=256, normalized=False):
    ...
    return cmap #size：38×3 or 150×3
...
class SemanticCloud:
    """
    输入rgb图像，做语义分割，生成带有语义标签颜色的图像（chair等）
    之后基于深度信息生成点云
    CNN: PSPNet (https://arxiv.org/abs/1612.01105)(resnet50) 在ADE20K数据集上预训练，在SUNRGBD数据集上优化调整
    """
    def __init__(self, gen_pcl = True):
        """
        Constructor
        \param gen_pcl (bool) 是否生成点云，如果生成则订阅深度图像
        """
        point_type = 1  #生成点云类型
        if point_type == 0:
        ...
        if self.point_type is not PointType.COLOR:
            ...
            # Setup model
            model_name ='pspnet'
            model_path = "/home/cbl/semantic_ws/src/semantic_slam/semantic_cloud/models/pspnet_50_ade20k.pth"
        ...
        if gen_pcl:
            ...
            self.depth_sub = message_filters.Subscriber("/camera/aligned_depth_to_color/image_raw", Image, queue_size = 1, buff_size = 40*480*640 ) # increase buffer size to avoid delay (despite queue_size = 1)
            ...
            self.image_sub = rospy.Subscriber("/camera/color/image_raw", Image, self.color_callback, queue_size = 1, buff_size = 30*480*640)
        print('Ready.')
