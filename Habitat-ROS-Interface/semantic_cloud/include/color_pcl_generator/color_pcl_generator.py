...
class ColorPclGenerator:
    ...
    def __init__(self, intrinsic, width = 640, height = 480, frame_id = "/camera_rgb_optical_frame", point_type = PointType.SEMANTICS_BAYESIAN):
        ...
        self.intrinsic = intrinsic # 固有
        self.num_semantic_colors = 3 # Number of semantic colors to be sent in the message 选了3个点
        # Allocate arrays
    ...
    def generate_cloud_data_common(self, bgr_img, depth_img):
        ...
        # print(np.max(self.ros_data[:,0][~np.isnan(self.ros_data[:,0])]),np.max(self.ros_data[:,1][~np.isnan(self.ros_data[:,1])]) )
        # print(np.min(self.ros_data[:,0][~np.isnan(self.ros_data[:,0])]),np.min(self.ros_data[:,1][~np.isnan(self.ros_data[:,1])]) )
    ...
    def generate_cloud_semantic_bayesian(self, bgr_img, depth_img, semantic_colors, confidences, stamp):
        ...
        for i in range(self.num_semantic_colors):
            ...
            self.semantic_colors_vect[:,4*i+2:4*i+3] = semantic_colors[i][:,:,2].reshape(-1,1) #(307200,12)
        # print("semantic_colors_vect",np.shape(self.semantic_colors_vect.view('<f4')))
        
        # Transform class confidence
        for i in range(self.num_semantic_colors):
            self.confidences_vect[:,i:i+1] = confidences[i].reshape(-1,1)
        # Concatenate data
        self.ros_data[:,8:8+self.num_semantic_colors] = self.semantic_colors_vect.view('<f4') # 以字节流的方式传送数据，要有偏移
        self.ros_data[:,12:12+self.num_semantic_colors] = self.confidences_vect
        return self.make_ros_cloud(stamp) # 以字节形式发送数据 stamp时间戳
