...
namespace octomap
{
  ...
  SemanticsBayesian SemanticsBayesian::semanticFusion(const SemanticsBayesian s1, const SemanticsBayesian s2)
  {
    ...
    std::sort(semantic_colors.begin(), semantic_colors.end()); // Asc order sorting 升序
    SemanticsBayesian ret;
    for(int i = 0; i < NUM_SEMANTICS; i++)
    {
      ret.data[i] = semantic_colors[semantic_colors.size() - 1 - i]; // Take last elements 选择概率最大的三个元素
    }
    return ret;
  }
