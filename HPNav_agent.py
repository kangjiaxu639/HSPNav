...
Transition = namedtuple('Transition',
        ('state', 'action', 'next_state', 'reward', 'global_graph', 'next_global_graph'))
...
class replay_buffer(object):
    ...
    def push(self, *args):
        if len(self.memory) < self.capacity:
            self.memory.append(None) # 添加一个空位 为后续数据的添加预留位置
        ...
        self.position = (self.position + 1) % self.capacity # 循环添加
...
class HPNavAgent:
    def __init__(
        ...
        if cmplt:
            ...
            self.cmplt_model.load_state_dict(torch.load(cmplt_pretrained), strict=False)
            ...
            if conf and not self.fake_conf:
                ...
                self.conf_model.load_state_dict(torch.load(conf_pretrained), strict=False)
        ...
        if discrete:
            ...
            = QNet(self.num_channel, (not att and conf) or self.fake_conf, 
                    rc=rc, preconf=preconf, hsp_layer_infos=hsp_layer_infos)
        ...
        if training:
            if discrete:
                ...
                = QNet(self.num_channel, (not att and conf) or self.fake_conf,
                        rc=rc, preconf=preconf, hsp_layer_infos=hsp_layer_infos)
            self.Q_t.load_state_dict(self.Q.module.state_dict(), strict=False)
            self.Q_t.eval()
        ...
        self.mapper = Mapper(self.device, 512, 512, 16, 16,
                    self.num_channel - 1, np.pi / 2., self.ignore)
        # self.mapper = Mapper(self.device, 512, 512, 12, 12,
        #                      self.num_channel - 1, np.pi / 2., self.ignore)
    ...
    def train_Q(self):
        ...
        non_final_mask, non_final_next_states, non_final_next_global_graphs = None, None, None
        try:
            ...
            non_final_next_global_graphs = [g for g in batch.next_global_graph if g is not None]
        except:
            pass
        state_batch = torch.cat(batch.state).to(self.device)
        reward_batch = torch.cat(batch.reward).to(self.device)
        action_batch = torch.cat(batch.action).to(self.device)
        # global_graph_list = [g for g in batch.global_graph]

        state_action_values = torch.zeros(self.batch_size, device=self.device)
        q_out = self.Q(state_batch, batch.global_graph)
        ...
        if non_final_mask is not None and non_final_next_states is not None:
            if self.double_dqn:
                with torch.no_grad():
                    best_action\
                    = self.Q(non_final_next_states, non_final_next_global_graphs).view(non_final_next_states.shape[0],
                            -1).max(1)[1].view(non_final_next_states.shape[0],
                                    1)
                    next_state_values[non_final_mask]\
                    = self.Q_t(non_final_next_states, non_final_next_global_graphs).view(non_final_next_states.shape[0],
                            -1).gather(1, best_action).view(-1)
            else:
                next_state_values[non_final_mask]\
        = self.Q_t(non_final_next_states, non_final_next_global_graphs).view(non_final_next_states.shape[0], -1).max(1)[0].detach()
        ...
        if self.user_semantics:
            ...
            with torch.no_grad():
                self.raw_semantics = self.seg_model(self.image,
                        self.depth).detach().cpu()[0] # [40, 480, 640]
    ...
    def single_view_and_save_new(self):
        ...
        quaternion\
        = self.env.sim.get_agent_state().sensor_states['depth'].rotation
        translation\
                = self.env.sim.get_agent_state().sensor_states['depth'].position
        observations\
        ...
        if self.user_semantics:
            self.mapper.append(quaternion, translation, observations,
                raw_semantics=self.raw_semantics) # 更新语义地图
        else:
            self.mapper.append(quaternion, translation, observations,
                raw_semantics=None)
    def view(self):
        if self.pano:
            ...
            self.single_view_and_save_new()
            self.single_view_and_save()
        node_features, edge_features, edge_idx= self.mapper.get_topo_global() #获取全局拓扑图
        # print(node_features.shape[0], edge_idx.shape[0])
        # node_features, edge_features, edge_idx = torch.rand((1,1,1)), torch.rand((1,1,1)), torch.rand((1,1,1))
        self.global_graph = {
        ...
        _, self.current_obs = self.memory.get_height_map(quaternion,
            translation, self.area_x, self.area_z, self.h, self.w) # （1，41，128，128）
        ...
        with torch.no_grad():
            if self.cmplt:
                ...
                if self.unconf:
                    ...
                    normed_cmplt = F.softmax(self.cmplted_obs, dim=1) # Tensor(1,41,128,128)
                # self.state(1,82,128,128) target_map(1,41,128,128)
                self.state = torch.cat((normed_cmplt, self.target_map), dim=1)
                if self.fake_conf:

                    self.conf_obs\
                            = (torch.argmax(self.current_obs,
                                    dim=1)!=self.num_channel-1).unsqueeze(1).float() # (1,1,128,128)
                  ...
                  #  self.conf_obs[..., torch.argmax(self.current_obs,dim=1) !=
                   #         self.num_channel - 1] = 1.
                    ...
                    if not self.att:
                        self.state = torch.cat((self.state, self.conf_obs), dim=1) #(1,83,128,128)
                    else:
    ...
    def step(self, randomness):
        ...
        if self.eps_len == self.max_step:
            return
        if self.discrete:
            self.q_map = self.Q(self.state.to(self.device), [self.global_graph]).detach().cpu()
            assert self.q_map.shape[0] == 1, 'make sure q_map size is 1'
            self.q_map = self.q_map[0]
            self.old_state = self.state.clone()
            self.old_global_graph = deepcopy(self.global_graph)
            ...
            if self.training:
                self.replay_buffer.push(self.old_state,
                    ...
                    self.done else None, torch.Tensor([reward]), self.old_global_graph, self.global_graph if not self.done else None)
            return reward
        # think about which point to go towards 
        with torch.no_grad():
            self.q_map\
            = self.Q(self.state.to(self.device), [self.global_graph]).detach().cpu()[0][0]
    ...
    def get_agent_current_locs(self):
        quaternion, translation = self.get_trinsics()
        global_z, global_x = self.mapper.get_map_location(translation, float(self.h),
                    float(self.w))

        o = self.mapper.get_orient(quaternion)
        return global_z, global_x, o
