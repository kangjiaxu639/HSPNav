...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        return {'image': torch.from_numpy(image).float(),
                ...
                'label': torch.from_numpy(label.astype(float)).float(),
                'label2': torch.from_numpy(label2.astype(float)).float(),
                'label3': torch.from_numpy(label3.astype(float)).float(),
                'label4': torch.from_numpy(label4.astype(float)).float(),
                'label5': torch.from_numpy(label5.astype(float)).float()}
