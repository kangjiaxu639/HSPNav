...
########################Navigation Result Visualization Tools########################
...
def visualization(rgb, semantic, global_map, pad = 20):
    # 图片添加黑框
    rgb = cv2.copyMakeBorder(rgb, 1, 1, 1, 1, cv2.BORDER_CONSTANT)
    global_map = cv2.copyMakeBorder(global_map, 1, 1, 1, 1, cv2.BORDER_CONSTANT)
    semantic = cv2.copyMakeBorder(semantic, 1, 1, 1, 1, cv2.BORDER_CONSTANT)
    width = pad + rgb.shape[0] + pad + global_map.shape[0] + pad
    height =pad + global_map.shape[0] + pad
    img = np.ones((width, height, 3)) * 255
    img[pad: pad + rgb.shape[0], pad:pad + rgb.shape[1], :] = rgb
    img[pad:pad + semantic.shape[0], pad + rgb.shape[1]: pad + rgb.shape[1] + semantic.shape[1]:, :] = semantic
    img[2 * pad + rgb.shape[0]:2 * pad + rgb.shape[0] + global_map.shape[0], pad:pad + global_map.shape[1], :] = global_map
    img = img.astype(np.uint8)
    cv2.putText(img, 'RGB', (pad, 13), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 0), 2)
    cv2.putText(img, 'LocalMap', (pad+rgb.shape[0], 13), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 0), 2)
    cv2.putText(img, 'GlobalMap', (pad, 2*pad+rgb.shape[1] - 5), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 0), 2)
    return img
