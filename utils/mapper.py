import sys
sys.path.append('/home/ros/kjx/HSPNav/utils')
import cv2
import random
import torch
...
from create_semantic_topo_vg import compute_topo_for_map
import skimage
...
class Mapper:
    ...
    def append(self, quaternion, translation, observations, raw_semantics=None):
        ...
        XZ = torch.cat((torch.cat((XZ_ff, XZ_fc), dim=0), 
            torch.cat((XZ_cc, XZ_cf), dim=0)), dim=0) # point clouds
    ...
    def get_map_location(self, translation, size_x=None, size_z=None, has_map=True):
    ...
    def get_topo_global(self):
        """Get the obejct relation graph based on the Global map"""
        node_features, edge_features, edge_idx = compute_topo_for_map(self.get_map_global().unsqueeze(0))
...
if __name__ == '__main__':
    ...
    config = habitat.get_config(config_paths="/home/ros/kjx/SSCNav/configs/agent_test.yaml")
    config.defrost()
